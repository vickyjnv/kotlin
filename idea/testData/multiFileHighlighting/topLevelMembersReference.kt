...
fun testWhere(list: List<Int>) {
    funWithWhere(1, list)
    funWithWhere(1, <error descr="[CONSTANT_EXPECTED_TYPE_MISMATCH] The integer literal does not conform to the expected type Collection<Any?>"><error descr="[CONSTANT_EXPECTED_TYPE_MISMATCH] The integer literal does not conform to the expected type Collection<TypeVariable(G)>">2</error></error>)
}
